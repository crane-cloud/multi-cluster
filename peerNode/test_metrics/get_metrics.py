...
def generic_get_metrics(ip, metric_name):
    ...
    response = requests.get(f"{base_url}?target={fomatted_ip}.{metric_name}&format=json")
    values =[]
...
def generic_get_direct_percentile_value(ip,metric_name):
    fomatted_ip = ip.replace('.','_').replace(':','_')
    response = requests.get(f"{base_url}?target=nPercentile({fomatted_ip}.{metric_name},95)&format=json")
    values =[]
    if response.status_code == 200:
        data = response.json()
...
def get_availability(ip):
    fomatted_ip = ip.replace('.','_').replace(':','_')
    response = requests.get(f"{base_url}?target=averageSeries({fomatted_ip}.Availability.A)&format=json")
    values =[]
    if response.status_code == 200:
        data = response.json()
        values = data[0]['datapoints']
